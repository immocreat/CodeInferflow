...
int InferenceEngine::AddQuery(int& token_count, const EncoderInput &encoder_input,
    ...
    TokenizationAlg alg)
{
    ...
    token_count = encoder_input_tokens.empty() ? decoder_prefix_tokens.size() : encoder_input_tokens.size();
    return query_id;
}
...
void InferenceEngine::BuildDecoderInput(DecoderPrefix &obj, const LlmQuery &query)
{
    ...
    while (start_pos < template_len)
    {
        ...
        case KeyId::QueryTextWithPreSpace:
            ...
            section_text += StringUtil::ToUtf8(query.text);
            break;
        case KeyId::CodePrefix:
            section_text += StringUtil::ToUtf8(query.code_prefix);
            break;
        case KeyId::CodeSuffix:
            section_text += StringUtil::ToUtf8(query.code_suffix);
            break;
        case KeyId::CodeLanguage:
            section_text += StringUtil::ToUtf8(query.code_language);
            break;
        case KeyId::CodeFilePath:
            section_text += StringUtil::ToUtf8(query.code_file_path);
            break;
        case KeyId::Bos:
            token_id = vocab.bos();
            is_as_token = true;
...
params:
    output: output
    perfstat:
    input: including input id, encoder/decoder
*/
    ...
    const LocalInput &input)
{
    ...
    // each worker in woker groups responsible for certain layers
    for (int worker_id = 0; worker_id < worker_num; worker_id++)
    {
        ...
        auto *worker_ptr = worker_group->workers[0]; // Always use workers 0?
        // cout << "Worker Group: " << worker_id << "Worker Group Count: " << worker_num << ", Workers Count: " << worker_group->workers.size() << endl;
        ...
        worker_ptr->Create(); // Create thread may be costly
        // auto end = std::chrono::high_resolution_clock::now();
        // auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        // std::cout << "Thread creation time: " << duration << " microseconds" << std::endl;
        worker_ptr->Join();
...
void InferenceEngine::InitKeyMap()
{
    ...
    key_map_["code_prefix"] = KeyId::CodePrefix;
    key_map_["code_suffix"] = KeyId::CodeSuffix;
    key_map_["code_language"] = KeyId::CodeLanguage;
    key_map_["code_file_path"] = KeyId::CodeFilePath;
}
