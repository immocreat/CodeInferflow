...
bool TensorOpr::Activation(DeviceTensor &B, const DeviceTensor &A, ActivationFn fn)
{
    ...
    case ActivationFn::GLU_SILU:
        ...
        break;
    case ActivationFn::GELU_TANH:
        ret = Gelu_TanhActivation(B, A);
        break;
    default:
...
bool TensorOpr::Gelu_TanhActivation(DeviceTensor &B, const DeviceTensor &A)
{
    bool ret = IsCompatible_AB(A, B);
    if (!ret) {
        return false;
    ...
    if (A.data_type == ElementType::F16)
    {
        ...
        {
            half *b_data = B.data_f16();
            GELU_TANHActivation_Kernel<half, half><<<grid, block>>>(
                M, N, a_data, b_data);
        }
        else
        {
            float *b_data = B.data_f32();
            GELU_TANHActivation_Kernel<half, float><<<grid, block>>>(
                M, N, a_data, b_data);
        }
    }
    else
    {
        ...
        GELU_TANHActivation_Kernel<float, float><<<grid, block>>>(
            M, N, a_data, b_data);
    }

    ret = CudaUtil::DeviceSynchronize("GELU_TanhActivation");
    return ret;
}
